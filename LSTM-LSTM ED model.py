# LSTM-LSTM Encoder-Decoder model
...
def model_architecture(X_train, y_train):
    ...
    model = Sequential()
    model.add(LSTM(48, activation='sigmoid', input_shape=(time_steps, features)))
...
'''
def train_model(X_train, y_train, verbose=1, epochs=50, batch_size=152):
...
def plot_predictions(predictions, dataset, input_size, output_size):
    ...
    pyplot.title("LSTM-LSTM Encoder-Decoder forecast")
    pyplot.legend()
    pyplot.show()
