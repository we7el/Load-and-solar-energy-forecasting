...
def model_architecture(X_train, y_train):
    ...
    model = Sequential()
    model.add(Conv1D(filters=48, kernel_size=2, activation='relu', input_shape=(time_steps, features)))
    model.add(MaxPooling1D(pool_size=2))
    model.add(Flatten())
    model.add(RepeatVector(output_size))
    model.add(LSTM(336, activation='sigmoid', return_sequences=True))
    model.add(TimeDistributed(Dense(336, activation='relu')))
...
'''
def train_model(X_train, y_train, verbose=1, epochs=100, batch_size=152):
    ...
    model.save('encoder_decoder_CNN_LSTM-LSTM.h5')
...
def plot_predictions(predictions, dataset, input_size, output_size):
    ...
    pyplot.title("CNN-LSTM Encoder-Decoder forecast")
    pyplot.legend()
    pyplot.show()
